...
function generate_unique_random($table , $con)
{
		...
		while($unique==0)
		{
			...
			for ($i = 0; $i < $length; $i++) {
			 ...
			 $sql="SELECT * FROM `$table` WHERE `dalily_token`='$randomString'";
						...
						if($query2) // will return true if succefull else it will return false
						 {
							 ...
							 if($rowcount2>=1)// found here
							 {
								  ...
								  $token =  generate_unique_random("admin",$con);
								 $_SESSION['dalily_token']=$token;
								
								 $sql = "UPDATE `admin` SET `dalily_token`='$token' WHERE `username`='$username'";
								 $query=mysqli_query($con,$sql);
...
background-repeat: no-repeat;">
    ...
    <div class="middle-box text-center loginscreen animated fadeInDown">
        <div class="login" style="direction: rtl;">
            ...
            <p>الرجاء تسجيل الدخول لإدارة جميع البيانات</p>
